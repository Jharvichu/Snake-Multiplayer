...
public class MessageProtocol {
    ...
    static String createConnectMessage(int playerId) {
        return "CONNECT" + SEP + playerId; // Mensaje conexión de jugador. Ejem: "CONNECT:12"
    }

    // Envía movimiento con dirección normalizada
    // CAMBIO: Solo enviar dirección, el servidor identifica jugador por conexión
    static String createMoveMessage(String direction, int playerId) {
        String dir = normalizeDirection(direction);
        return "MOVE" + SEP + dir; // MOVE:UP (servidor ya conoce el playerId)
    }
